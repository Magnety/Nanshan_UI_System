...
from multi_view.SCTransformer.SCTransformer import build_sc_t
import csv
...
class Solver(object):
    ...
    def build_model(self):
        ...
        elif self.model_type == 'SCTransformer':
    ...
    def train(self):
        ...
        unet_path = os.path.join(self.model_path, '%s-%d-%.6f-%d-%.4f-%s.pkl' % (
        self.model_type, self.num_epochs, self.lr, self.num_epochs_decay, self.augmentation_prob,self.curtime1))
        ...
        if os.path.isfile(unet_path):
            ...
            for epoch in range(self.num_epochs):
                ...
                if (epoch + 1) > (self.num_epochs - self.num_epochs_decay):
                    lr -= (lr / float(self.num_epochs_decay)*2.5)
                    for param_group in self.optimizer.param_groups:
                        param_group['lr'] = lr
